...
public class UserCommJobWriter implements ItemWriter<UserCommunication>,
	...
	private static final Logger logger = Logger
			.getLogger(UserCommJobWriter.class);
		...
		for (UserCommunication oUserComm : oCommunications) {
			...
			if (null != oUser && null != templates && !templates.isEmpty()) {
				for (CommJobTemplate oTemplate : templates) {
					...
					logger.info("Processing User type : " + oUser.getUserType()
							+ " ,UserId: " + oUser.getId() + " , Template: "
					...
					if (oCommTemplate.getCommChannelId() == oEmailChannel
						...
						try {
							...
							logger.info("Sending Email for User type : "
							...
							oMail.setTemplateName(oCommTemplate
									.getTemplateFileName());
							oMail.setMailTo(oUser.getEmailId());
							oMail.setMailFrom(mailFromAddress);
							oMail.getMapInlineImages().put("image1",
									oCommJob.getImageUrl());
							emailGateway.sendMail(oMail);
							CommTemplateBuffer.getJobStatusMap()
									.get(oCommJob.getCommJobId())
						...
						logger.info("Sending SMS for User type : "
								+ oUser.getUserType() + " ,UserId: "
								+ oUser.getId() + " , Job Id : "
								+ oCommJob.getCommJobId());
					}
